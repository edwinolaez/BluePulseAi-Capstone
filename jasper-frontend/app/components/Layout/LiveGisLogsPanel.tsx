...
const MESSAGE_POOL: { level: Level; message: string }[] = [
  { level: "INFO",    message: "Precipitation database sync: 0.0mm rainfall logged in Jasper." },
  { level: "SUCCESS", message: "Erosion Risk Model run completed. Raster files rendered to Map instance." },
  { level: "WARN",    message: "Watershed Hydration index indicating rapid drainage in Segment 4B." },
  { level: "ERROR",   message: "Failed telemetry query at Silt Monitor S-3. Retrying in 15 seconds." },
  { level: "INFO",    message: "Polling active IoT station: 'IoT Jasper-A1'." },
  { level: "SUCCESS", message: "Sentinel-2 multi-spectral bands synced (98% confidence)." },
];
...
export function LiveGisLogsPanel({ open, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-[2000] w-full max-w-md flex flex-col bg-gray-950 border-l border-gray-800 shadow-2xl">
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800">
        <div className="flex items-center gap-2 text-gray-100">
          <TerminalIcon className="w-5 h-5" />
          <h2 className="text-base font-bold">Live GIS Logs</h2>
        </div>
      ...
      <div className="flex items-center justify-between gap-2 px-5 py-3 border-b border-gray-800">
      ...
      <div ref={listRef} className="flex-1 overflow-y-auto px-5 py-3 flex flex-col gap-3">
        {logs.map((log) => (
          <div key={log.id} className="flex items-start gap-2.5 text-xs">
            <span className="font-mono text-gray-500 shrink-0 pt-0.5">{log.time}</span>
      ...
      <div className="px-5 py-2.5 border-t border-gray-800 text-[11px] text-gray-500 text-center">
        Raster Pipeline Ingest Rate: 1.2 MB/s &nbsp;|&nbsp; Buffer Size: {logs.length}
      </div>
    </div>
  );
}
